...
public class MinRefils {
    private static int getTotalRefills(int[] water,int andyCapacity,int berryCapacity,int n){
        int totalRefills = 0;

        int left = 0 , right = n - 1;
        int a = andyCapacity , b = berryCapacity;
        while(left < right){
            if(a <  water[left]){
                totalRefills++;
                a = andyCapacity;
        ...
        if(left == right){
            if (Math.max(a, b) < water[left]) {
                totalRefills++;   // Refill if neither can serve
            }
        }
        return totalRefills;
    }
    public static void main(String[] args) {
        ...
        for(int i = 0 ; i < n ; i++) water[i] = sc.nextInt();

        int andyCapacity = sc.nextInt();

        int berryCapacity = sc.nextInt();

        System.out.print(getTotalRefills(water,andyCapacity,berryCapacity,n));
