 ...
 */
import java.util.*;
public class Remove_Ones_Flips {
    private static boolean canRemoveAll1sByFlippingRowsOrCols(int[][] grid,int m,int n){
        // first flip all the cols where 1 is the tehre 

        for(int j = 0 ; j < n ; j++){
            if(grid[0][j] == 1){
                // flip all bits of this col
                for(int i = 0 ; i < m ; i++){
                    grid[i][j] = 1 - grid[i][j];
                }
            }
        ...
        // now traverse from first row 
        for(int i = 1 ; i < m ; i++){
            int sum = 0;
            for(int j = 0 ; j < n ; j++){
                sum += grid[i][j];
            }
            // if sum is not zero and not sum is not equal to number of cols then return false 
    ...
    public static void main(String[] args) {
        ...
        System.out.println(canRemoveAll1sByFlippingRowsOrCols(grid,m,n));
