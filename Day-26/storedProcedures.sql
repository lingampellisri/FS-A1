...
DROP PROCEDURE IF EXISTS getEmpByDeptAndComm;
DELIMITER $$

CREATE PROCEDURE getEmpByDeptAndComm(
  IN p_deptNo INT, 
  IN p_comm DECIMAL(10,2))
BEGIN
  SELECT * 
  FROM emp
  WHERE deptno = p_deptNo AND comm = p_comm;
END$$

DELIMITER ;

CALL getEmpByDeptAndComm(10, 500.00);
CALL getEmpByDeptAndComm(20, 300.00);
# ------------------------------------------
