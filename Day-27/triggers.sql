...
DROP TRIGGER update_customers_validate;
DELIMITER //
CREATE TRIGGER update_customers_validate
BEFORE UPDATE 
ON CUSTOMERS FOR EACH ROW 
BEGIN 
    if(OLD.age < 20)
    THEN 
        SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: Old Age is lesser than 20';
    ELSEIF(NEW.age < 20)
    THEN 
        SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: Age cannot be lesser than 20';
	END IF;
...
DELIMITER ;
 ...
 DELIMITER //
 CREATE TRIGGER before_insert_dept_pk
BEFORE INSERT
 ON dept FOR EACH ROW
 BEGIN
 IF NEW.deptno IN (SELECT dept.deptno FROM dept) THEN 
 SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: Cannot insert duplicate into dept';
 END IF;
 END //
 DELIMITER ;
 
DELIMITER //
 CREATE TRIGGER before_update_dept_pk
BEFORE UPDATE
 ON dept FOR EACH ROW
 BEGIN
 IF NEW.deptno IN (SELECT dept.deptno FROM dept) THEN 
 SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Warning: Cannot update due to duplicacy';
 END IF;
 END //
 DELIMITER ;
